...
class DataExportController extends Controller
{
     ...
     *
     * @param Request $request The incoming request with parameters.
     *
     * @return JsonResponse
     */
    public function index(Request $request): JsonResponse
    {
        $type = $request->get('type', 'contests');

        return match ($type) {
            'contests'     => $this->exportContests(),
            'votes'        => $this->exportVotes(),
            'entries'      => $this->exportEntries(),
            'vote-ratings' => $this->exportVoteRatings(),
            default        => response()->json(['error' => 'Invalid export type'], 400),
        };
    }

    /**
     * Export a single contest
     *
     * @param Contest $contest The contest to export.
     *
     * @return JsonResponse
     */
    public function show(Contest $contest): JsonResponse
    {
        return $this->exportSingleContest($contest);
    }
     ...
     * @return JsonResponse
     */
    private function exportContests(): JsonResponse
    {
     ...
     * @return JsonResponse
     */
    private function exportVotes(): JsonResponse
    {
     ...
     * @return JsonResponse
     */
    private function exportEntries(): JsonResponse
    {
     ...
     * @return JsonResponse
     */
    private function exportVoteRatings(): JsonResponse
    {
     ...
     * @return JsonResponse
     */
    private function exportSingleContest(Contest $contest): JsonResponse
    {
        ...
        $data = [
            ...
            'entries'        => $contest->entries->map(function ($entry) {
                return [
                    ...
                    'average_rating'    => $entry->averageRating,
                    'total_votes'       => $entry->voteRatings->groupBy('vote_id')->count(),
                    'ratings_breakdown' => $entry->voteRatings->groupBy('rating_factor_id')
                        ->map(function ($ratings, $factorId) {
